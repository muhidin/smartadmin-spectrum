...
function sas_customize_register( $wp_customize ) {
    ...
    $wp_customize->add_setting( 'sas_header_display_type', array(
        'default'           => 'both',
        'sanitize_callback' => 'sas_sanitize_header_display',
    ) );
    $wp_customize->add_control( 'sas_header_display_type_control', array(
        'label'    => esc_html__( 'Header Display Type', 'smartadmin-spectrum' ),
        'section'  => 'sas_header_section',
        'settings' => 'sas_header_display_type',
        'type'     => 'select',
        'choices'  => array(
    ...
    $wp_customize->add_setting( 'sas_header_layout_position', array(
        'default'           => 'brand-stacked',
        'sanitize_callback' => 'sas_sanitize_layout_position',
    ) );
    $wp_customize->add_control( 'sas_header_layout_position_control', array(
        'label'    => esc_html__( 'Logo & Text Position', 'smartadmin-spectrum' ),
        'section'  => 'sas_header_section',
        'settings' => 'sas_header_layout_position',
        'type'     => 'radio',
        'choices'  => array(
    ...
    for ( $i = 1; $i <= 5; $i++ ) {
        $wp_customize->add_setting( 'sas_hero_slide_' . $i, array(
            'default'           => get_template_directory_uri() . '/images/slider' . $i . '.jpg',
            'sanitize_callback' => 'esc_url_raw',
        ) );

        $wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'sas_hero_slide_' . $i . '_control', array(
            'label'    => sprintf( esc_html__( 'Slide Image %d', 'smartadmin-spectrum' ), $i ),
            'section'  => 'sas_slider_section',
            'settings' => 'sas_hero_slide_' . $i,
        ) ) );
    }
}
 ...
 */
function sas_register_block_styles() {
...
add_action( 'init', 'sas_register_block_styles' );
 ...
 */
function sas_filter_theme_description( $description ) {
    return sas_get_theme_description();
}
add_filter( 'theme_description', 'sas_filter_theme_description' );

/**
 * Add custom action to display theme description in admin
 */
function sas_display_theme_description() {
        ...
        if ( in_array( $current_lang, array( 'id_ID', 'id', 'indonesian' ) ) ) {
            ...
            add_filter( 'wp_prepare_themes_for_js', function( $themes ) {
                if ( isset( $themes['smartadmin-spectrum'] ) ) {
                    $themes['smartadmin-spectrum']['description'] = sas_get_theme_description();
                }
                return $themes;
            });
        }
...
add_action( 'admin_init', 'sas_display_theme_description' );
